...
(
	...
	-- 가게 이미지 경로 ( 원본 )
	STORE_IMG_ORIGIN_PATH varchar2(255) NOT NULL,
...
ALTER TABLE YM_RESERVATION_PRODUCT
	...
	REFERENCES YM_PRODUCT (PRODUCT_NO) ON DELETE CASCADE
;

ALTER TABLE YM_PRODUCT
	ADD FOREIGN KEY (PRODUCT_CATEGORY_NO)
	REFERENCES YM_PRODUCT_CATEGORY (PRODUCT_CATEGORY_NO) ON DELETE CASCADE
;

ALTER TABLE YM_RESERVATION_PRODUCT
	ADD FOREIGN KEY (RESERVATION_NO)
	REFERENCES YM_RESERVATION (RESERVATION_NO) ON DELETE CASCADE
;

ALTER TABLE YM_BOARD
	ADD FOREIGN KEY (USER_NO)
	REFERENCES YM_USER (USER_NO) ON DELETE CASCADE
;
